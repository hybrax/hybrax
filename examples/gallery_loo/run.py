...
print(
    f"final holdout (run_3)   = {result.holdout_loss_by_step[last_step]:.4f}"
...
ax.semilogy(
    range(1, len(result.mean_loss_by_step) + 1),
    result.mean_loss_by_step,
    label="train (all 3 processes)",
)
ax.semilogy(
    steps,
    [result.holdout_loss_by_step[s] for s in steps],
    "o--",
    label="holdout (run_3)",
)
ax.set_xlabel("step")
