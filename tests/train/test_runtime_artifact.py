...
_TEMPERATURE_VALUES = jnp.asarray([30.0, 30.5, 31.0])


def _process(name: str, feed_concentration: float, parent: str | None = None):
    process_type = AugmentedBioProcess if parent is not None else BioProcess
    process_kwargs = {"parent_process": parent} if parent is not None else {}
    feed_medium = FeedMedium(
        name="feed",
        density=1.0,
        density_unit="kg/L",
        components={
            component: FeedMediumComponent(
                name=component,
                unit="g/L",
                concentration=StaticVariable(
                    feed_concentration if component == "substrate" else 0.0
                ),
                is_controlled=False,
            )
            for component in ("biomass", "substrate", "product")
        },
    )
    return process_type(
        ...
        volume=Volume(
            ...
            volume_changes={
                ...
                "feed_B": Inflow(
                    ...
                    is_controlled=False,
                    is_continuous=True,
                    values=TimeSeries(times=[0.0, 1.0], values=[0.0, 0.02]),
                    feed_medium=feed_medium,
                ),
                "bleed": Outflow(
                    name="bleed",
                    ...
                    is_controlled=False,
                    is_continuous=True,
                    values=TimeSeries(times=[0.0, 1.0], values=[0.0, -0.01]),
            ...
            components={
                "biomass": ReactorMediumComponent(
                    ...
                    concentration=TimeSeries(
                        times=[0.0, 0.5, 1.0], values=[0.1, 0.15, 0.2]
                    ),
                ),
                "substrate": ReactorMediumComponent(
                    ...
                    unit="g/L",
                    concentration=TimeSeries(
                        times=[0.0, 0.5, 1.0], values=[20.0, 15.0, 10.0]
                    ),
                ),
                "product": ReactorMediumComponent(
                    ...
                    unit="g/L",
                    concentration=TimeSeries(
                        times=[0.0, 0.5, 1.0], values=[0.0, 0.5, 1.0]
                    ),
                ),
            },
        ),
        process_variables={
            "temperature": ProcessVariable(
                ...
                values=TimeSeries(
                    times=_FEED_TIMES,
                    values=_TEMPERATURE_VALUES,
                    poly=PPoly.from_samples_pchip(_FEED_TIMES, _TEMPERATURE_VALUES),
                    segment_start_piece_idx=jnp.asarray([0], dtype=jnp.int32),
                ),
            )
        },
...
def _make_runtime_artifact_collection() -> BioProcessCollection:
    processes = {
        "DoE1_R1": _process("DoE1_R1", 100.0),
        "DoE1_R2": _process("DoE1_R2", 200.0),
        "DoE1_R1__aug_000": _process("DoE1_R1__aug_000", 100.0, parent="DoE1_R1"),
        "DoE3_R4": _process("DoE3_R4", 300.0),
    }
...
@cache
def _source_collection() -> BioProcessCollection:
    return _make_runtime_artifact_collection()
