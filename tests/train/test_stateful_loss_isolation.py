...
def test_target_state_indices_exclude_latent_and_decoupled_h0_leaves_loss_unchanged():
    ...
    t_eval = jnp.asarray([0.0, 0.5], dtype=jnp.float32)

    wrapper_a = build_stateful_wrapper(
        process,
        ZeroLatentDerivativeModule(jnp.asarray([0.0], dtype=jnp.float32)),
    )
    wrapper_b = build_stateful_wrapper(
        process,
        ZeroLatentDerivativeModule(jnp.asarray([10.0], dtype=jnp.float32)),
    )
