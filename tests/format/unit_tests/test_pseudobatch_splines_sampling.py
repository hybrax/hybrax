...
# ---------------------------------------------------------------------------
# Test 4: Continuous Outflow (perfusion/continuous harvest) is unsupported
# ---------------------------------------------------------------------------


def _continuous_outflow_process(*, retention):
    """Classic fed-batch fixture plus one continuous Outflow, retained or not."""
    rm = ReactorMedium(
        name="medium",
        density=1.0,
        density_unit="kg/L",
        components={
            "biomass": ReactorMediumComponent(
                name="biomass",
                unit="g/L",
                concentration=_ts([0.0, 3.0, 6.0], [10.0, 10.0, 10.0]),
            ),
            "glucose": ReactorMediumComponent(
                name="glucose",
                unit="mmol/L",
    ...
    return BioProcess(
        metadata=BioProcessMetadata(
            name="continuous_outflow", process_type="fed_batch"
        ),
        time_axis=TimeAxis(unit="h", start=0.0, end=6.0, time_reference="inoculation"),
        volume=vol,
        reactor_medium=rm,
...
def test_continuous_outflow_raises_not_implemented(retention):
    ...
    not the retention value) — both build_pseudobatch_transform and
    build_pseudobatch_inputs must refuse it rather than produce wrong
    numbers."""
    proc = _continuous_outflow_process(retention=retention)

    with pytest.raises(NotImplementedError, match="continuous Outflow"):
        build_pseudobatch_transform(proc, ["glucose"])

    with pytest.raises(NotImplementedError, match="continuous Outflow"):
        build_pseudobatch_inputs(proc, "glucose")
