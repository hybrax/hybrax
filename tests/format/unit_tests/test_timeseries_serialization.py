        ...
        reactor_medium=ReactorMedium(
            ...
            components={
                "biomass": ReactorMediumComponent(
                    ...
                    pseudobatch_concentration=_c_star_ts(),
                ),
            },
        ),
        process_variables={
...
def test_roundtrip_uses_canonical_timeseries_shape() -> None:
    ...
    assert "pseudobatch_concentration" in component_payload
...
@pytest.mark.parametrize("field", ["pseudobatch_concentration", "total_volume"])
def test_optional_schema_fields_may_be_absent(field: str) -> None:
    process = _build_process()
    if field == "pseudobatch_concentration":
        process.reactor_medium.components["biomass"].pseudobatch_concentration = None
    else:
    ...
    if field == "pseudobatch_concentration":
        ...
        assert "pseudobatch_concentration" not in component_payload
        assert (
            loaded.processes["p"]
