...
- `sim_1_intracellular` keeps those structural ideas but changes the product
  biology to exercise bp-format intracellular state handling.
...
```bash
pixi run python tests/e2e_tests/sim_1/simulation.py --output-dir tests/e2e_tests/sim_1/data --plot-dir tests/e2e_tests/sim_1/data/simulation_plots
```

`simulation.py` writes:

- `data/simulation_dense_output.csv`: dense online rows,
  offline rows, event-boundary rows, controls, diagnostics, and simulated rate
  columns (`q_*`, `r_*`).
- `data/simulation_events.csv`: event rows keyed by `process_id`.
- `data/simulation_plots/*.png`: simulation plots.
...
- `data/process_collection.json`: `BioProcessCollection` with `sim_1_run_1` and
  `sim_1_run_2`.

It is generated by parsing `data/simulation_dense_output.csv` and
`data/simulation_events.csv` with `load_utils.py`.
