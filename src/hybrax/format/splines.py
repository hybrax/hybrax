...
- Pseudobatch transform computation (pseudobatch concentration, ADF, feed
  correction)
...
Design goals:
- Always compute pseudobatch transform (pseudobatch concentration) and
  correction terms from physical event semantics.
- Avoid dense-grid pseudo-events from interpolation artifacts.
- Fit:
    - pseudobatch concentration: smoothing TimeSeries spline where possible
...
def _reject_continuous_outflow(process: BioProcess) -> None:
    ...
    for vc_name, vc in process.volume.volume_changes.items():
        if isinstance(vc, Outflow) and vc.is_continuous:
            raise NotImplementedError(
                f"Pseudobatch normalization does not support continuous Outflows "
                f"(found {vc_name!r}, retention={vc.retention!r}). Its exact "
                "closed-form ADF construction assumes reactor volume only grows "
                "from Inflows; a continuous Outflow (perfusion, continuous "
                "harvest/bleed) would silently corrupt the transform regardless "
                "of retention. Only Inflow and discrete Outflow (sampling) "
...
def build_pseudobatch_inputs(process: BioProcess, species_name: str) -> Dict[str, Any]:
    ...
    """
    _reject_continuous_outflow(process)
...
def build_pseudobatch_transform(
    ...
    *,
    ...
    for name in selected_species:
        ...
        component.pseudobatch_concentration = c_star_ts
...
def _evaluate_pseudobatch_concentration(value: Any, t_eval: jnp.ndarray) -> jnp.ndarray:
    ...
    if not isinstance(value, TimeSeries):
        raise TypeError(
            "Pseudobatch evaluation requires component.pseudobatch_concentration "
...
def evaluate_pseudobatch_transform(
    ...
    cs = _evaluate_pseudobatch_concentration(c_star, t_eval)
    if t_eval.ndim == 0:
...
def build_backtransform_spline(
    ...
    supplies the stored ``pseudobatch_concentration`` carrier.
    """
    transform = getattr(process, "pseudobatch_transform", None)
    if transform is None:
        raise ValueError("process.pseudobatch_transform is required.")
    ...
    rep = component.pseudobatch_concentration
    if rep is None:
        raise ValueError(
            f"Pseudobatch species {species_name!r} has no pseudobatch_concentration; "
    ...
    if isinstance(rep, StaticVariable):
        ...
        if not _has_spline_state(rep):
            raise ValueError(
                "Pseudobatch pseudobatch_concentration must carry stored spline state; "
                "regenerate the pseudobatch transform."
            )
        c_star_ts = rep
