...
def _compute_dense_process_export(
    ...
    *,
    ...
    SCL_pseudo_y0 = process_wrapper.initial_pseudo_state_from_raw(
        process_data.y0_measured
    )
    ...
    for process_name in selected_processes:
        ...
        if render_plots:
            ...
            for i, sp_name in enumerate(modeled_RMC_names):
                ...
                _sp_loss = (
                    process_named_losses.get(sp_name) if process_named_losses else None
                )
            ...
            for k, fn in enumerate(modeled_FVC_names):
                ...
                _b_loss = (
                    process_named_losses.get(_b_label) if process_named_losses else None
                )
