...
raise — see [Design Rationale §6](01_design_rationale.md#6-check-the-data-then-fail-loudly).
...
`FAIL volume_change_sign: 'feed' (FeedVolumeChange) contains negative values; feed volume changes must have all values >= 0`
...
9. `validate_initial_state_alignment`
...
| Message | Cause | Fix |
|---------|-------|-----|
| `FAIL volume_change_sign: ... contains negative values` | Sign convention flipped | Feeds are ≥ 0, samples ≤ 0 |
...
| `FAIL volume_change_states: ... missing feed components for state variable(s)` | Feed medium omits a reactor species | Add it, with the real concentration — `0.0` only if truly absent |
