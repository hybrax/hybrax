...
def _simulate_modeled_pv(x0: float, glyco0: float, t: np.ndarray) -> dict[str, np.ndarray]:
...
def build_demo_modeled_pv() -> None:
    ...
    for name, (x0, glyco0) in MPV_RUNS.items():
        ...
        feed_medium = hxf.FeedMedium(
            name="dilution_feed", density=1.0, density_unit="kg/L",
            components={
                "biomass": hxf.FeedMediumComponent(
                    name="biomass", unit="g/L", concentration=hxf.StaticVariable(0.0)),
            },
        ...
        process = hxf.BioProcess(
            ...
            time_axis=hxf.TimeAxis(unit="h", start=0.0, end=MPV_T_END,
                                  time_reference="inoculation"),
            volume=hxf.Volume(
                initial_volume=MPV_V0, unit="L",
                volume_changes={
                    "dilution_bolus": hxf.Inflow(
                        name="dilution_bolus", unit="L", is_controlled=True,
                        is_continuous=False,
                        values=TimeSeries(times=np.array([MPV_T_BOLUS]),
            ...
            reactor_medium=hxf.ReactorMedium(
                ...
                components={
                    "biomass": hxf.ReactorMediumComponent(
                        name="biomass", unit="g/L",
                        concentration=TimeSeries(times=MPV_SAMPLE_TIMES.astype(float),
                                                 values=meas["biomass"].astype(float)),
                        bounds=(0.0, None),
                    ),
                },
            ...
            process_variables={
                "glyco_frac": hxf.ProcessVariable(
                    name="glyco_frac", unit="-", is_controlled=False,
                    values=TimeSeries(times=MPV_SAMPLE_TIMES.astype(float),
    ...
    hxf.serialization.save_process_collection(collection, out / "data.json")
    (out / "ground_truth.json").write_text(json.dumps({
        "q_biomass": MPV_Q_BIOMASS, "r_glyco_frac": MPV_R_GLYCO_FRAC,
    }, indent=2) + "\n")
