...
`_eyring_rate_ceiling` is an algebraically reparameterized Eq. 4e, vectorized
over its three instances. It combines the Eyring transition-state factor with
a reversible-inactivation equilibrium factor and returns temperature-dependent
maximum prefactors for uptake and production and a minimum prefactor for
maintenance. `_inhibition_product`/`_activation_product` are the paper's own
...
specifically: reversible inactivation has reduced the temperature-dependent
uptake prefactor enough that feed exceeds cellular uptake and glucose
concentration rises.

## What Did It Learn About the Eyring Parameters?
...
`Teq` locates the midpoint of the reversible-inactivation equilibrium, where
the active-enzyme fraction in `_eyring_rate_ceiling` is one half. For example,
the rendered uptake fit is 31.7°C versus a true `Teq` of 35.9°C, a 4.2°C
difference. Trajectories can fit despite this mismatch because `A`, `Ea_R`,
`Teq`, and `dHeq_R` jointly shape the temperature response. Estimating their
individual values requires experiments that distinguish the activation and
reversible-inactivation regimes.

## Gotchas

- **Eyring parameters are jointly identified by the temperature response.**
  `A`, `Ea_R`, `Teq`, and `dHeq_R` can compensate for one another over a narrow
  temperature range. Use experiments that distinguish the activation and
  reversible-inactivation regimes before interpreting individual values.
